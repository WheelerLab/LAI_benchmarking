...
if [[ -f $NAT && $CEU && $YRI ]]; then
    ...
    (/home/rschubert1/scratch/time -v ./elai-lin -g ${out}_NAT.recode.geno.txt -p 10 -g ${out}_CEU.recode.geno.txt -p 11 -g ${out}_YRI.recode.geno.txt -p 12 -g ${out}.recode.geno.txt -p 1 -pos ${out}.recode.pos.txt -C 3 -o ${out}) 2> ${out}_benchmarking.txt
elif [[ -f $NAT && $CEU ]]; then
