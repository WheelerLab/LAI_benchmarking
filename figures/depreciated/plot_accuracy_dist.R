...
for (i in c("3WAY","HIS","AFA")){
  sub<-all_software_long %>% filter(pop==i)
  model<-lm(value~software,data=sub)
  A<-aov(model)
  tukey<-TukeyHSD(A)
  print(tukey)
  # setEPS()
  # postscript("Z:/data/Local_ancestry_project/redoancestry/" %&% i %&% "_Tukey.eps")
  png("Z:/data/Local_ancestry_project/redoancestry/" %&% i %&% "_Tukey.png")
  par(mar=c(5,8,4,1)+.1)
  plot(tukey, las=1)
  title("\n" %&% i)
  dev.off()
...
setEPS()
postscript("Z:/data/Local_ancestry_project/redoancestry/accuracy_dist_2.eps")
