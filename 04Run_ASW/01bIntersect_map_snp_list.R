...
parser$add_argument("--thin", help="number of snps to output after selection. Randomly selects snps.")
...
if (!is.null(args$thin)){
  thinned<-intersection[sample(nrow(intersection), size = args$thin),]
  new_list<-select(thinned,V1) %>% as.data.frame()
  fwrite(thinned, args$out %&% "_genetic_map_intersection.txt", col.names = F, row.names = F,quote = F,sep=' ')
