...
(~/scratch/time -v -o ${outDir}_benchmarking.txt loter_cli -r ${outDir}*_ref.vcf -a ${query} -f vcf -o ${outDir}_results.txt -n 1 -v)
(~/scratch/time -v -o ${outDir}_pc_benchmarking.txt loter_cli -pc -r ${outDir}*_ref.vcf -a ${query} -f vcf -o ${outDir}_results.txt -n 1 -v)
